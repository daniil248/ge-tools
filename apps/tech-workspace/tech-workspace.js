...
function generateReportBlocks(v, B) {
  ...
  blk.push(B.caption(`Концепция объекта ЦОД · Вариант «${v.name}»${v.primary ? ' (основной)' : ''} · сформировано ${date} · Технолог ЦОД, GE Tools`));
  ...
  blk.push(B.caption(`Документ сгенерирован автоматически в GE Tools · Технолог ЦОД · ${date}`));
  return blk;
}
