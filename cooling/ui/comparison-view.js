...
export function renderComparisonTable(metrics, currency = '₽') {
  ...
  const ROWS = [
    ...
    { id: 'fcHours',   label: 'FC часов/год',                 tip: 'Часы в году с T_наружн < freeCoolingThreshold (proxy по primary spec). Точные числа — во вкладке energy.', fmt: (m) => `${(m.fc.fcHours || 0).toFixed(0)}`,                cmp: 'higher', getter: (m) => m.fc.fcHours || 0 },
    { id: 'fcPct',     label: '% года в FC',                  tip: 'Доля года в FC (proxy).',                                                                       fmt: (m) => `${(m.fc.fcPct || 0).toFixed(1)} %`,                cmp: 'higher', getter: (m) => m.fc.fcPct || 0 },
    { id: 'energy',    label: 'Эл. потребление СИСТЕМЫ',     tip: 'Σ кВт·ч/год по всем активным единицам (cold-резерв = 0; hot-резерв делит нагрузку).',          fmt: (m) => fmtKwh(m.fc.energyKwh),                              cmp: 'lower',  getter: (m) => m.fc.energyKwh },
    { id: 'opex',      label: 'OPEX за год (электр.)',       tip: 'Годовые затраты системы на электричество (energy × тариф).',                                    fmt: (m) => fmtM(m.fc.costRub),                                  cmp: 'lower',  getter: (m) => m.fc.costRub },
