...
// ─── v0.59.990: Chiller energy stacked bar chart
// X = T_bin (°C), Y = годовая энергия (кВт·ч/год).
// Stack 1 (red) = P_mech × hours — компрессорная составляющая.
// Stack 2 (green) = P_aux × hours — насосы/вентиляторы FC.
// Визуально мгновенно видно где free-cooling работает (зелёные бары
// слева, красные справа) и сравнить «мех vs FC» вклад в годовое.
export function drawChillerEnergyChart(cvs, rows) {
  ...
  for (const r of rows) {
    ...
    labels.push(r.tBin);
    // P_mech × hours (компрессор) и P_aux × hours (FC насосы/вентиляторы)
    // Из row: capacity, copMech, fcFraction, hours.
  ...
  register(cvs, new Chart(cvs, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: 'Free-cooling aux (насосы/вент)',
          data: auxKwh,
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            ...
            footer: (items) => {
              ...
              return `Σ за бин: ${total.toFixed(0)} кВт·ч/год`;
            },
        ...
        title: {
          display: true,
          text: 'Годовое эл. потребление по бинам T_amb (стек: компрессор + aux)',
