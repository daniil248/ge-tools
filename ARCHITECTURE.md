...
> база ВСЕХ типов схем; `js/calc`/`js/methods` — calc-модуль класса
> `lib/*-methods`. **D2** модули могут нативно использоваться в др.
> модулях (нативный импорт calc-модуля допустим). **D3** Конструктор —
> универсальная база ВСЕХ типов схем с контекстом систем (ОТМЕНЯЕТ
> v0.60.277/278 «Конструктор = чистая электрика»). **D4** каждый
> обособленный метод (стандарт/методика) — отдельный файл + picker.
> Лакмус «CORE-calc vs calc-lib» ниже подлежит переписи в X.4.5; до
> этого описанная раскладка действует как есть.
