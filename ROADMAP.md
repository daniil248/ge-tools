...
> **Статус:** v0.60.562 (2026-05-17). Текущий приоритет — **архитектурная
...
- [~] **1.7** Все отчёты только через `reports/` (blocks API).
  ...
  pdu-config ✅ (v0.60.562 → blocks[]); осталось
  `battery/battery-calc.js:3787`, `logistics/logistics.js:222`,
  `ups-config/ups-config.js:2221` — по одному (визуальный diff-парность).
