...
> **Статус:** v0.60.673 (2026-05-17). Модуль отчётов (X.6.3)
> **завершён**: flow-документ (наложение устранено), редактор на
> flow (WYSIWYG=PDF), правка из превью, общий `composeReport` —
> ВСЕ отчёты; **+ Word-style разделы**: обложка + sectionBreak с
> переменной ориентацией/геометрией (preview/PDF/DOCX), verified
> e2e; R6 (чистка legacy) опц. Архитектурная
