...
>
> **Статус:** v0.60.764 (2026-05-18). Платформа + предметные модули.
...
- [~] **6.2 Матрица ответственности разделов (ГИП).** Запрос
  ...
  аддитивно, отсутствие=own, exports-only cache-safe). B — ГИП-UI в
  карточке проекта (селектор режима + поля требований, гейт роль
  gip/manager/owner, бейджи на разделах). C (позже) — read-only
  enforcement в модулях (баннер требований вместо полной разработки).
