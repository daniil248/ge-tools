...
> **Статус:** v0.60.528 (2026-05-16). Текущий приоритет — **архитектурная
...
> 1-й calc-lib `suppression-methods` зарегистрирован (modules.json
> v1.2.0). Осталось (R2, advisory): projects/project.js (15+ чужих
> LS) + `*-bridge.js`; синк `requires` + регистрация 13 ui (Фаза 1).
...
- [~] X.1.4 Структура-канон + calc/ui-нормализация. memory:module_separation
  ...
  + CI non-blocking. `suppression-methods` зарегистрирован как 1-й
  `kind:'calc-lib'` (manifest+REGISTRY_ORDER+modules.json v1.2.0,
  паритет OK, нулевой риск). Осталось: синк `requires` (Фаза 1),
  регистрация 13 ui-папок (по одной), дробление монолитов (calc→ui).
