...
> Ф-E (всё) + Ф-F ПОЛНОСТЬЮ (scs оболочка F1-F4) ✅;
> Ф-G (агрегация BOM) по плану. Модуль отчётов (X.6.3)
...
- [ ] X.4.5 **Архитектурные дополнения Пользователя (2026-05-17)** —
  ...
  - [~] X.4.5.3 D3: Конструктор — ОДИН app, единый интерфейс для
    ...
    аккордеон · F3 role-gate · F4 отчёты). Дальше Ф-G — технолог:
    единая агрегация BOM по всем дисциплинам через реестр объектов.
